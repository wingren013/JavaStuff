...
public class genmeth {
	...
	public static void generateRandomArray(int x, int y){
		int size = 0;
		int a = 0;
		int b= 0;
		if(x > y){
		...
		for(int i=0; i < size; i++){
			series[i] = (int) (a+Math.random()*(b-a+1));
		}
	}
	public static void print_array(int[] series){
	...
	public static void generateRandomArrayExclusive(int x, int y){
		int size = 0;
		int a = 0;
		int b= 0;
		int n =0;
	...
	public static void printRandomArrayEntry(int[] series, int size){
		int n =0;
		for(int i=0; i < size; i++){
			n = (int) (1+Math.random()*(series.length));
			if(duplicate(series, i, n)==true){
				i += -1;
			}else{
